(function () {
    ...
    function initStarPreview() {
        ...
        function getMetrics() {
            ...
            return { width: rect.width, height: rect.height, centerX, centerY, radius };
        }
        ...
        function spawnParticle() {
            ...
            let y = 0;
            if (side === 0) {
                x = randomBetween(0.05, 0.95) * m.width;
                y = -14;
            } else if (side === 1) {
                x = m.width + 14;
                y = randomBetween(0.08, 0.92) * m.height;
            } else if (side === 2) {
                x = randomBetween(0.05, 0.95) * m.width;
                y = m.height + 14;
            } else {
            ...
            const speed = randomBetween(70, 125);
        ...
        function updateVisuals() {
            starHue = lerp(starHue, targetHue, 0.075);
            ...
            const glowA = 42 + pulse * 18;
            const glowB = 92 + pulse * 34;
            core.style.boxShadow = '0 0 ' + glowA.toFixed(1) + 'px rgba(255, 197, 96, ' + (0.42 + pulse * 0.18).toFixed(3) + '), 0 0 ' + glowB.toFixed(1) + 'px rgba(255, 145, 56, ' + (0.18 + pulse * 0.12).toFixed(3) + ')';
        }
        ...
        function frame(time) {
            ...
            for (let i = particles.length - 1; i >= 0; i -= 1) {
                ...
                if ((particle.vx >= 0 && particle.x >= particle.impactX) ||
                    (particle.vx < 0 && particle.x <= particle.impactX) ||
                    Math.hypot(dx, dy) <= particle.size * 0.55) {
